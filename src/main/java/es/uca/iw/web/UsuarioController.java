...
public class UsuarioController {
    ...
    public String adscritos(@PathVariable("id") Long id, Model uiModel) {
        // Lista los usuarios interesados en la oferta {id}
        Oferta oferta = Oferta.findOferta(id);
        // Busca todas las peticiones de esa oferta
        List<PeticionOferta> peticiones = PeticionOferta.findPeticionOfertasByOferta(oferta).getResultList();
        List<Usuario> usuarios = null;
        for(PeticionOferta peticion:peticiones)
            usuarios.add(peticion.getUsuario_demandante());
